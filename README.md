...
Locally: Using docker:
    ...
    docker run --interactive --tty --publish 8080:8080 gorenje/erlang-red:0.3.3
...
| Node | Comment | Example Flow |
| ---- | ------- | ------------ |
| [process](src/nodes/ered_node_erlprocess.erl) | Erlang-Red representation of an existing Erlang process. The process node can be sent messages which are then converted to Erlang messages and sent to the process | TBD |
| [captureio](src/nodes/ered_node_erlcaptureio.erl) | Node that can be used to capture output generated by processes that would normally end up on the console. Output is passed out as messages to be handled with in a flow, much as the catch node does with exceptions. | TBD |
