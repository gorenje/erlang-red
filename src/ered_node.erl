...
handle_call(Msg, _From, {Module, NodeDef}) ->
    io:format(
        ...
        [self(), maps:get(<<"type">>, NodeDef), Msg]
    ),
    {reply, NodeDef, {Module, NodeDef}}.
...
handle_cast({outgoing = MsgType, Msg}, {Module, NodeDef}) ->
    %% also generate a msg trace on outgoing messages - makes it obvious what
    %% is happening in a flow.
...
handle_cast(Msg, {Module, NodeDef}) ->
    io:format(
        "Node ~p, ~p Received unsupported cast {{{ ~p }}}~n",
        [self(), maps:get(<<"type">>, NodeDef), Msg]
    ),
...
handle_info({tcp_comm, Tuple}, {Module, NodeDef}) ->
    NodeDef2 = Module:handle_event(Tuple, NodeDef),
    {noreply, {Module, NodeDef2}};
%% tcp event generated by a tco request node and represents incoming data
%% from the server end.
handle_info({tcp, Socket, Data}, {Module, NodeDef}) ->
    NodeDef2 = Module:handle_event({tcpr_data, Socket, Data}, NodeDef),
    {noreply, {Module, NodeDef2}};
%% tcp_request is sent this when the connection is dropped by the server
handle_info({tcp_closed, _Socket} = Event, {Module, NodeDef}) ->
    NodeDef2 = Module:handle_event(Event, NodeDef),
    {noreply, {Module, NodeDef2}};
%%
...
handle_info({function_eval_result = MsgType, Msg, _O} = Evt, {Module, NodeDef}) ->
    ...
    handle_msg_responder(MsgType, Msg, Module, Results, post_completed);
%%
%% Generated by the kafka manager when a client becomes available
handle_info(
    {kafka_client_check, _KafkaCLientId} = Evt,
    {Module, NodeDef}
) ->
    NodeDef2 = Module:handle_event(Evt, NodeDef),
    {noreply, {Module, NodeDef2}};
%%
%% Generated by kafka brod producer
handle_info(
    {brod_produce_reply, _BrodCallRef, BaseOffset, Result} = Evnt,
    {Module, NodeDef}
) ->
    NodeDef2 =
        Module:handle_event(
            {kafka_produce_reply, BaseOffset, Result, Evnt},
            NodeDef
        ),
    {noreply, {Module, NodeDef2}};
%% Generated by the brod consumer, format defined by brod.
handle_info(
    {_NodePid, {kafka_message_set, _Topic, _Partition, _Offset, _MsgLst} = Evt},
    {Module, NodeDef}
) ->
    NodeDef2 = Module:handle_event(Evt, NodeDef),
    {noreply, {Module, NodeDef2}};
...
handle_info(Event, {Module, NodeDef}) ->
    io:format(
        "Node ~p, ~p Received unsupported info {{{ ~p }}}~n",
